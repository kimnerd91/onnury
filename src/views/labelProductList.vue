<template>
  ...
  <section class="section-shop padding-tb-50" style="padding-bottom: 20px">
    <div class="container">
...
export default {
  ...
  methods: {
    ...
    async onedepthProductSListUp(str) {
      try {
        ...
        if (this.categoryListSelect.length > 0) {
          this.paramsValue.relatedMiddleCategoryId = this.categoryListSelect.toString();
        }
        ...
        const response = await axios.get(
          `/api/product/page/label/${this.id}/list`,
          {
        ...
        this.productList = response.data.data.labelProductList;
        this.total = response.data.data.totalLabelProductCount;
    ...
    filterByPrice() {
      // Validate input values before calling the method
      if (this.startRangePrice && this.endRangePrice) {
        this.onedepthProductSListUp("srch");
      } else {
        console.warn("가격 범위를 올바르게 입력하세요."); // 유효성 검사 메시지
      }
    },
    isChecked(brandId) {
...
console.log(sresponse);
        this.productList = sresponse.data.data.labelProductList;
        this.total = sresponse.data.data.totalLabelProductCount;
        this.paging.maxpage = Math.ceil(this.total / 20);

        // // Extract unique brands using Set
        // const uniqueBrands = new Set(this.productList.map((product) => product.brand));
        this.filterbrandList = sresponse.data.data.brandList; // Convert Set back to Array

        // // Extract unique middle categories using Set
        // const uniqueCategories = new Set(
        //   this.productList.map((product) => product.middleCategory)
        // );
        
        this.filterrelatedUnderCategoryList = sresponse.data.data.middleCategoryList; // Convert Set back to Array
